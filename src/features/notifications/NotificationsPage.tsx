...
export function NotificationsPage():JSX.Element{
  ...
  useEffect(()=>{
    let active=true;
    listNotifications().then(data=>{if(active){setRows(data);setLoading(false);}}).catch(err=>{if(active){setError(err instanceof Error?err.message:'Failed to load notifications');setLoading(false);}});
    return()=>{active=false;};
  },[]);
  async function open(row:NotificationRow){
    ...
    }catch(err){setError(err instanceof Error?err.message:'Failed to open notification');}
  }
  return(
    <IonPage>
      <IonHeader><IonToolbar><IonTitle>Notifications</IonTitle></IonToolbar></IonHeader>
      <IonContent fullscreen>
        <div className="cs-page">
          {error&&<p role="alert" style={{color:'var(--cs-danger)'}}>{error}</p>}
          {loading&&<div className="cs-card" aria-busy="true">Loading notifications…</div>}
          {!loading&&rows.length===0&&<div className="cs-empty"><p>No notifications yet.</p></div>}
