...
export function AdminPage(): JSX.Element {
  ...
  async function load() {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to load moderator console');
    } finally {
      setLoading(false);
    }
  }
      ...
      setStats((current) =>
        current
          ? {
              ...current,
              openReports:
                reports.filter((r) => r.id !== report.id && ['open', 'reviewing'].includes(r.status)).length +
                (status === 'open' || status === 'reviewing' ? 1 : 0),
            }
          : current,
      );
    } catch (err) {
  ...
  return (
    <IonPage>
      <IonHeader>
        <IonToolbar>
          <IonTitle>Moderator Console</IonTitle>
        </IonToolbar>
      </IonHeader>
      <IonContent fullscreen>
        <div className="cs-page">
          ...
          <section className="cs-card" style={{ marginTop: 'var(--cs-space-3)' }}>
            ...
            {reports.length === 0 ? (
              <p className="cs-muted">No reports.</p>
            ) : (
                    ...
                    <div className="cs-row" style={{ justifyContent: 'space-between' }}>
                      <div>
                        <div className="cs-strong">
                          {report.entity_type} · {report.reason}
                        </div>
                    ...
                    <div className="cs-row">
                      {report.status === 'open' && (
                        <IonButton
                          ...
                          onClick={() => void setStatus(report, 'reviewing')}
                        >
                          Review
                        </IonButton>
                      )}
...
function Stat({ label, value }: { label: string; value: number }): JSX.Element {
  return (
    <div>
      ...
      <div className="cs-muted">{label}</div>
    </div>
  );
}
