...
export function TripPage(): JSX.Element {
  ...
  async function refreshComparison(){
    if(!selectedIds.length){setError('Select at least one warehouse.');return;}
    setBusy(true);setError(null);
    try{setComparison(await compareTrip(selectedIds));}
    catch(err){setError(err instanceof Error?err.message:'Failed to compare warehouses');}
  ...
  return (
    <IonPage>
      <IonHeader><IonToolbar><IonTitle>Trip Mode</IonTitle></IonToolbar></IonHeader>
      <IonContent fullscreen>
        <div className="cs-page">
          <section className="cs-card">
            <h2 className="cs-section-title" style={{marginTop:0}}>Shopping list</h2>
            {loading&&<p className="cs-muted" aria-busy="true">Loading…</p>}
            ...
            <ul className="cs-stack" style={{listStyle:'none',padding:0}}>
              {items.map(item=>(
                <li key={item.id} className="cs-row" style={{justifyContent:'space-between',alignItems:'center'}}>
                  <div className="cs-row" style={{flex:1}}>
                    ...
                    />
                    <button className="cs-link-button" onClick={()=>history.push(`/product/${item.product_id}`)}>
                  ...
                  <IonInput
                    ...
                    inputMode="decimal"
                    style={{maxWidth:80}}
                    onIonBlur={e=>void updateQuantity(item,String((e.target as HTMLIonInputElement).value??item.quantity))}
                  />
                  <IonButton size="small" fill="clear" onClick={()=>void remove(item)}>Remove</IonButton>
                </li>
              ))}
            </ul>
          </section>
          ...
          <section className="cs-card" style={{marginTop:'var(--cs-space-3)'}}>
            <h2 className="cs-section-title" style={{marginTop:0}}>Compare warehouses</h2>
              ...
              {warehouses.map(warehouse=>(
                <IonItem key={warehouse.id}>
                  <IonCheckbox
                    ...
                    onIonChange={e=>setSelectedIds(ids=>e.detail.checked?[...new Set([...ids,warehouse.id])]:ids.filter(id=>id!==warehouse.id))}
                  />
                  <IonLabel>{warehouse.name} · {warehouse.city}, {warehouse.state}</IonLabel>
                </IonItem>
              ))}
            </div>
            <IonButton expand="block" disabled={busy||activeItems.length===0} onClick={()=>void refreshComparison()}>
          ...
          {comparison.length>0&&(
            <section className="cs-card" style={{marginTop:'var(--cs-space-3)'}}>
              <h2 className="cs-section-title" style={{marginTop:0}}>Trip comparison</h2>
              <ul className="cs-stack" style={{listStyle:'none',padding:0}}>
                {comparison.map(row=>(
                  <li key={row.warehouse_id} className="cs-row" style={{justifyContent:'space-between'}}>
                    <div>
                      ...
                      <div className="cs-muted">{row.priced_items} of {row.list_items} active items priced</div>
                    </div>
                    <div className="cs-price cs-strong">{formatUSD(cents(Number(row.known_basket_cents)))}</div>
                  </li>
                ))}
              </ul>
            </section>
