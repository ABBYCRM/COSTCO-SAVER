...
export async function markNotificationRead(id:string):Promise<NotificationRow>{
  const result=await apiFetch<{notification:NotificationRow}>(`/api/v1/notifications/${id}/read`,{method:'PATCH'});
  return result.notification;
}
export async function registerDeviceToken(input:{platform:'ios'|'android'|'web';token:string;appVersion?:string|null}):Promise<void>{
